#' Implement the Particle Gibbs for the agent-based SIS model
#'
...
PG_SIS <- function(X, y, opt = Opt(), hypers = NULL, ini_param = NULL, fixed_gam = NULL,
  ...
  for(imcmc in 1 : num_mcmc){
    ...
    CBPF_result <- CBPF(y, config, num_particles);

    # Sample a particle proportional to weights
    ind <- sample(num_particles, 1,  prob = CBPF_result$weight)
    selected <- CBPF_result$particles[, , ind]
