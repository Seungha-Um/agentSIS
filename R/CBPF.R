#' Conditional Bootstrap Particle Filter for agent-based SIS model
...
CBPF <- function(y, model_config, num_particles){
  ...
  alphats <- matrix(model_config$alpha0, nrow = model_config$N, ncol = (num_particles-1));
  est_agent <- matrix(NA, nrow=N, ncol = num_observations)
  xts <- (runif((num_particles-1) * model_config$N) < alphats);
  xts <- cbind(xts, model_config$fix_agent[,1])
  its <- colSums(xts)
  ...
  for (t in 2 : num_observations){
    ...
    xts[,num_particles] <- model_config$fix_agent[,t]
    its <- colSums(xts)
